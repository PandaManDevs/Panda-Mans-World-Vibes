...
class Music(commands.Cog):
    ...
    async def make_source(self, track: Track) -> discord.AudioSource:
        ...
        if not direct_url:
            # Re-extract if the direct URL has expired.
        ...
        if not direct_url:
            raise ValueError("The audio stream URL could not be extracted.")

        logger.info(
            "Starting FFmpeg playback: guild=%s source=%s url=%s",
            track.requested_by.guild.id,
            track.source_name,
            direct_url[:180],
        )

        # Decode the remote stream to PCM. This is intentionally done through
        # FFmpeg instead of passing a web/HLS URL directly to Discord.
